...
- "Name: Mingze Gao"
- "UID: 3035426241"
subtitle: "Association of invasive meningococcal disease and environmental and virologic exposure"
output:
  html_document:
    df_print: paged
---
...
```{r Read Data}
library(readr)
IMD <- read_csv("CMED6020 Assignment1 - IMD.CSV")
```
...
```{r}
summary(model_poi <- glm(imd ~ flu + maxtemp + log10(pop), data = IMD, family = poisson()))
```
...
```{r}
require(MASS)
summary(model_nb <- glm.nb(imd ~ flu + maxtemp + log10(pop), data = IMD))
```
...
We can present the weekly counts of IMD for flu, which is 4.638 (95% CI: 0.618 - 34.772).
